...
/*   Updated: 2022/07/21 17:38:18 by keaton           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_pinfo_init(t_pinfo *pinfo, int *info)
{
	...
	pthread_mutex_init(&(pinfo->lock_full), NULL);
	pthread_mutex_init(&(pinfo->lock_mt), NULL);
	pinfo->meals = 0;
	pinfo->full = 0;
	pinfo->ph_nbr = -1;
	if ((pinfo->sem_fork == (sem_t *)-1 || pinfo->sem_possible == (sem_t *)-1
			|| pinfo->sem_report == (sem_t *)-1) && ft_close_sems(pinfo))
